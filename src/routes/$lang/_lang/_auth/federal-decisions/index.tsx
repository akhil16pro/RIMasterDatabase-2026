...
function RouteComponent() {
  ...
  return (
    <DashboardLayout isLoading={false} title={t("all_federal_decisions")}>
      <motion.div
      ...
      >
        <div className="md:flex-1">
          <DefaultButton
            ...
            onClick={() => {
              navigate({
                to: "/" + i18n.language + "/federal-decisions/add",
              });
            }}
          />
        </div>
...
function PageTable({ search }: { search: string }) {
  ...
  const { data, isLoading, error, isRefetching } = useQuery({
    queryKey: ["federalDecisionTable", pagination.currentPage, i18n.language],
    enabled: !!userSession?.accessToken,
    ...
    queryFn: async () => {
      try {
        const res = await apiClient
          .get(
            i18n.language +
              `/federal-decision/table?page=${pagination.currentPage}`,
          )
          .json<any>();
        console.log("FEDERAL_DECISION_TABLE_DATA", res?.data);
        return res?.data;
      } catch (error) {
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: async ({ slug, status }: { slug: string; status: number }) => {
      return await apiClient
        .post(`${i18n.language}/federal-decision/status/${slug}/${status}`)
        .json<any>();
    },
    onSuccess: (res: any) => {
      toast.success(res?.message || t("success"));
      queryClient.invalidateQueries({ queryKey: ["federalDecisionTable"] });
    },
    onError: (error: any) => {
      console.error(error);
...
function ViewAction({ slug }: { slug: string }) {
  ...
  const handleView = () => {
    navigate({
      to: `/${i18n.language}/federal-decisions/view/${slug}`,
      params: {
        slug: slug,
      },
    });
...
function EditAction({ slug }: { slug: string }) {
  ...
  const handleEdit = () => {
    navigate({
      to: `/${i18n.language}/federal-decisions/edit/${slug}`,
      params: {
        slug: slug,
      },
    });
...
function DeleteAction({
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        const res = await apiClient
          .get(i18n.language + "/federal-decision/delete/" + value.slug)
          .json<any>();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <DialogContent className="lg:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("delete_federal_decision")}</DialogTitle>
          </DialogHeader>
          <div className="flex">
