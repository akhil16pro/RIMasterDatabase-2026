...
function RouteComponent() {
  ...
  return (
    <DashboardLayout isLoading={false} title={t("all_international_treaties")}>
      <motion.div
  ...
  const { data, isLoading, error, isRefetching } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const res = await apiClient
          .post(i18n.language + `/international-treaty/table`, {
            headers: {
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: async ({ slug, status }: { slug: string; status: number }) => {
      return await apiClient
        .post(`${i18n.language}/international-treaty/status/${slug}/${status}`)
        .json<any>();
    },
    onSuccess: (res: any) => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <DialogContent className="lg:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("delete_international_treaty")}</DialogTitle>
          </DialogHeader>
          <div className="flex">
